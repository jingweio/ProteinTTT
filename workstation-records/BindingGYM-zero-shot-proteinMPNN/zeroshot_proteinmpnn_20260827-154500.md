...
（created 2026-08-27 15:45; status: **S1–S2 完成，S3/S5 运行中**）
...
### 5.3 三个口径的对照，以及那条预测的检验

| | 25-assay mean Spearman | Δ vs 官方 |
|---|---|---|
| 官方发布（M=5，单次未设种） | 0.396950 | — |
| 上次复现（**M=1**，Ibex a100） | 0.384029 | **−0.012921** |
| **本次复现（M=5，seed 1）** | **0.391356** | **−0.005594** |

⇒ **聚合层面弥合了缺口的 57%**，方向与预测一致。

**⚠️ 但机制未被证实。** 同 seed、同 assay 的**配对**对照（S1 探针那 3 个）：

| assay | M=1 | M=5 | Δ(M5−M1) |
|---|---|---|---|
| 4D5_HER2 | 0.3261 | 0.3267 | +0.0006 |
| HLA-A2_TAPBPR | 0.4154 | 0.4116 | −0.0038 |
| Z-ZpA963_HL2 | 0.3751 | 0.3774 | +0.0023 |
| **均值** | 0.3722 | 0.3719 | **−0.0003** |

**配对差几乎为零。** 所以那 57% 的弥合**可能主要是 seed 运气，而不是 M 的系统效应** ——
上次那次是**另一台机器、另一个 env、另一个（未设种的）seed**，不是一个受控对照。

⇒ **判决：预测在聚合层面弱支持，在配对检验上不支持。** 已加跑 **S5**：在 10 个最便宜的 assay 上
补 M=1 臂（seed 1），做一个 10-assay 的干净配对对照来定案。
（注：早前在 BH3 上测到的 M=1 0.6667 vs M=5 0.6817 是**另一套条件** —— soluble 权重 + backbone
noise 0.1 + 8 replicates，不能直接搬到这里。）

### 5.4 S3 — seed 方差（进行中，目前 2 个 seed × 5 个共同 assay）

| assay | seed1 | seed2 | sd |
|---|---|---|---|
| BH3_Bcl-xL_1PQ1 | 0.6533 | 0.6440 | 0.0066 |
| BH3_Mcl-1_3KZ0 | 0.6514 | 0.6644 | 0.0092 |
| PSD95_CRIPT_1BE9 | 0.3677 | 0.3813 | 0.0097 |
| PSD95_Tm2F_1BE9 | 0.1943 | 0.2014 | 0.0050 |
| Z-ZpA963_HL2_2M5A | 0.3774 | 0.3192 | **0.0412** |

per-assay sd：median **0.0092**，max 0.0412。子集均值的 sd = **0.0048**。
⇒ S2 的残差 −0.0056 约为 **1.2σ**，与"参考值是单次抽样"完全一致。
（n=2 的 sd 估计很脆，等 5 个 seed 齐了替换。）

...
**尚未完成**：`lasermpnn_ar` 9/25。它只影响"LASErMPNN 在两个口径下排序是否一致"这一项交叉验证，
**不改变上述任何结论** —— 而且该问题**已用已完成的 9 个 assay 得到肯定回答**（§7.5，ρ = 0.967）。
**该 run 已于 2026-09-13 13:48 停止** —— 它卡在 `GB1_IgG-Fc`（n=92,891）上无限重启，
连续 3 个 job / 6+ 小时零产出（§7.5）。**不影响任何结论。**
...
### 7.5 LASErMPNN 的双口径一致性（部分，n=9）

`lasermpnn_ar` 仍在跑（9/25）。但**用已完成的 9 个 assay 就能做配对对照**，零额外计算：

```
LASErMPNN   A vs B:  mean Δ = -0.0182   3/9 正号   p = 0.3594   排序一致性 ρ = 0.9667
ProteinMPNN A vs B（同样这 9 个）: mean Δ = -0.0108   4/9   p = 0.6523   ρ = 0.8500
```

**两个口径在 LASErMPNN 上无显著差异，且 per-assay 排序一致性 ρ = 0.967 —— 比 ProteinMPNN
自己的 0.850 还高。** 即"LASErMPNN 在两个口径下排序是否一致"这个问题，
**在这 9 个 assay 上已得到肯定回答**。

⚠️ **限定**：这 9 个**不是随机子集**，是按 assay index 顺序先跑到的那几个
（4 个 Z-domain + 2 个 5A12 + CXCR4 + 4D5_HER2 + hYAP65），**规模偏小**。
剩余 16 个里包含 `GB1_IgG-Fc`(n=92,891)、`KRAS_PICK3CG-RBD`(L=1,107) 这类大的，
不能排除大 assay 上两口径分歧更大的可能。

**🔴 为什么停止：它结构上跑不完，不是"要不要花 40 小时"的问题**

先前估的"约 35–40 GPU-小时"**是错的** —— 那个估算假设它在稳步推进。实际查证：

```
job 51817366   TIMEOUT  03:00:15   完成 0 个 assay
job 51828713   TIMEOUT  03:00:14   完成 0 个 assay
job 51828718   RUNNING  00:26:58   (同样会 TIMEOUT)
```

**连续三个 job、累计 6+ 小时，零产出。** 根因：
下一个待跑的是 `GB1_IgG-Fc_1FCC`（**n = 92,891**, L = 262），
而 LASErMPNN 的 AR 口径是**每个 variant 一次 forward**（等变全原子 GNN）
⇒ 单个 assay 就需 ~9 小时，**超过任何一个 slot 的 walltime**。

打分脚本是**按 assay 原子写盘**的（跑完整个 assay 才写 csv），所以：
> job 超时 → 该 assay 的工作全部丢弃 → 下一个 job 从头重跑同一个 assay → 再超时 …
> **无限循环，永远停在 9/25，持续烧 GPU。**

⇒ **已停止该 job 链**（2026-09-13 13:48）。这不是成本取舍，是这条路径在当前
walltime 策略下**根本走不到终点**。

**若将来要补完**，需要先改设计，至少三选一：
1. 单 job 给足 walltime（GB1 一个就要 ~9 h，全量 ~40 h）—— 但我方 fairshare 下长 walltime 排队极久；
2. 把打分脚本改成 **assay 内分块 checkpoint**（每 N 个 variant 落盘一次），让超时可续；
3. 只在 AR 口径下跑**小 assay 子集**，明确标注覆盖率。

**当前不做**：§7.5 的 ρ = 0.967 已回答核心问题，且 §8 的三条结论都不依赖它。

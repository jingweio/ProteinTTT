...
| 问题 | 回答 |
|---|---|
| 界面效应是渐变还是阈值？ | **渐变**（§6）。5 档下真值 ρ_bin 在 16/16 为负；把 5 Å 两侧各自内部单独看，趋势依然显著。二值标签是有损摘要 —— 分档解释的方差是二值的 **2.1×**。 |
...
> 本节 2026-09-12 新增。§4/§5 用的是 `d ≤ 5 Å` 的**二分类**。它能说「两组有差异但重叠 0.697」，
...
| K = 2（§4/§5） | K = 5（本节） |
|---|---|
| Cliff's **δ**（两组谁高） | **ρ_bin** = Spearman(档序号, score)，负 = 越近越差 |
| **η²**（二值标签解释的方差） | **η²**（5 档解释的方差） |
| 两组的分布重叠 | 5 条分布曲线的逐档位移 |

### 6.3 口径：**16 个 assay 可用，但与 §4/§5 的 16 个成员不同**

判据：**≥3 档**且**每档 ≥30**。25 个里 **9 个不可用** —— 它们的 `d` 取值太少，切不出档：

| assay | n | `d` 的不同取值数 | 实际切出的档数 |
|---|---:|---:|---:|
| 4D5_HER2_fitness_1N8Z | 2,079 | 6 | 1 |
| 5A12_Ang2_fitness_4ZFG | 943 | 9 | 2 |
| 5A12_VEGF_fitness_4ZFF | 29,980 | 9 | 2 |
| BH3_Bcl-xL_normed_1PQ1 | 517 | 10 | 4 |
| BH3_Mcl-1_normed_3KZ0 | 517 | 9 | 4 |
| GB1_IgG-Fc_fitness_1FCC_2016 | 22,175 | 4 | 1 |
| Z-domain_ZSPA-1_LL1_fitness_1LP1 | 45,476 | 7 | 2 |
| Z-domain_ZSPA-1_LL2_fitness_1LP1 | 5,583 | 6 | 2 |
| Z-domain_ZpA963_HL1_fitness_2M5A | 2,903 | 5 | 2 |

⚠️ **可用的 16 个与 §4/§5 的「可检验 16 个」交集只有 15 个：**

- **只有分档能做**：`Z-domain_ZpA963_HL2` —— 它**全部 variant 都碰界面**，二值指示量恒定，
  但**界面内部的远近仍有区分度** ⇒ 分档视角能触及二值视角结构上够不到的 assay。
- **只有二值能做**：`5A12_Ang2` —— `d` 只切得出 2 档。

（★ = complexTTT 阶段排除的两个 assay，见 `Sources/datasets/BindingGYM-issues/`；本节沿用本文档 25-assay 口径，把它们一并列出并标记。）

### 6.4 结果

![graded trend](fig_graded_bins_trend.png)

***Figure 5.** 每条灰线一个 assay，粗线是中位数；纵轴是该档分数的**平均秩**（0.5 = 无效应），
所以跨 assay 可比。**A = 真值，B = ProteinMPNN 预测。***

![graded per assay](fig_graded_bins_per_assay.png)

***Figure 6.** 逐 assay 的 5 条分布曲线，蓝 = 最远、红 = 最近，虚线为各档中位数；面板按 ρ_bin 排序。*
...
| KRAS_DARPinK27_norfitness_5O2S ★ | 19,532 | 5 | **-0.279** | +0.123 | 0.102 | 0.022 | 3/4 | 2/4 |
| Z-domain_ZpA963_HL2_fitness_2M5A | 599 | 3 | **-0.276** | +0.056 | 0.112 | 0.003 | 1/2 | 0/2 |
...
| KRAS_SOS1_norfitness_8BE4 ★ | 19,424 | 5 | **-0.179** | +0.093 | 0.047 | 0.031 | 2/4 | 2/4 |
...
1. **真值上是一条清晰的单调梯度。ρ_bin 在 16/16 个 assay 上全为负**（中位 **−0.275**）。
   平均秩的中位轨迹 **0.622（B1）→ 0.389（B5）**；相邻档「往下走」的步数占 **72%**（随机是 50%）。
2. **分档比二值多解释一倍的方差**：真值 η² 中位 **0.105**，而二值 η² 中位 **0.051** ⇒ **2.1×**。
   **这是「二值标签是有损摘要」的直接量化。**
3. **🔴 是渐变，不是阈值 —— 决定性的子检验**：把 5 Å 两侧各自内部单独看，趋势**依然存在**：

   | 子集 | assay 数 | Spearman(`d`, DMS) 中位 | 方向一致 | p<0.05 |
   |---|---:|---:|---:|---:|
   | **只看 `d > 5 Å`**（全是「不碰」） | 15 | **+0.196** | 12/15 | **15/15** |
   | **只看 `d ≤ 5 Å`**（全是「碰」） | 16 | **+0.087** | 11/16 | 12/16 |

   ⇒ **在「不碰」那一组内部，越远仍然越好；在「碰」那一组内部，越浅仍然越好。**
   5 Å 只是穿过一条连续关系的一刀，**不是一个真实的分界**。
4. **ProteinMPNN 的那条线是平的。** ρ_bin 中位 **+0.009**，且 **8/16 为正**（方向相反）；
   平均秩轨迹 0.499 → 0.527，几乎不动；下降步数占比 54%（≈随机）。
   η² 中位 **0.027**，是真值的 **1/4**。
5. **逐 assay 仍然对不上**：Spearman(ρ_bin 真值, ρ_bin MPNN) = **+0.200, p = 0.458**。
   与 §5.3 的二值结论一致 —— 模型不只是幅度小，**在哪些 assay 上敏感也与真实情况无关**。
6. **一个诚实的限定**：逐档中位数**严格单调**的只有 4/16。梯度在**秩相关**意义上稳定
   （ρ_bin 16/16 为负），但逐档中位数会小幅抖动 —— 所以应当读作「单调趋势」，不是「逐档严格递减」。

**对本文档主结论的影响**：§4 的「界面标签只解释 5% 方差」是**二值口径**下的数字。
换成分档，真值的可解释方差翻倍到 **10.5%** —— 效应仍然不大，

...
| **ρ_w（连续）** | **ridge regression（α=1）** | **软标签 `w(r) = exp(−d(r)/5Å)`** | **预测值与真实 `w(r)` 之间的 Spearman 秩相关** |
...
| assay | L | 排除 | 参与探针的残基 | 界面比例 | AUC within | AUC LOAO | ρ_w within | ρ_w LOAO |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `5A12_Ang2_fitness_4ZFG` | 652 | 4 | 648 | 0.056 | 0.875 | 0.879 | 0.458 | 0.100 |
| `ACE2_SARS2-RBD_enrich_6M17` | 931 | 0 | 931 | 0.046 | 0.951 | 0.970 | 0.474 | 0.322 |
| `CD19_FMC63_Fitness_7URV` | 497 | 52 | 445 | 0.092 | 0.911 | 0.886 | 0.554 | 0.204 |
| `CXCR4_CXCL12_enrich_8U4O` | 360 | 0 | 360 | 0.169 | 0.820 | 0.851 | 0.537 | 0.405 |
| `GB1_IgG-Fc_fitness_1FCC` | 262 | 0 | 262 | 0.137 | 0.946 | 0.940 | 0.565 | 0.468 |
| `HLA-A2_TAPBPR_meanscore_5WER` | 644 | 62 | 582 | 0.124 | 0.891 | 0.897 | 0.639 | 0.466 |
| `KRAS_PICK3CG-RBD_norfitness_1HE8` | 1107 | 192 | 915 | 0.038 | 0.918 | 0.946 | 0.262 | 0.166 |
| `KRAS_RAF1-RBD_norfitness_6VJJ` | 245 | 0 | 245 | 0.127 | 0.829 | 0.968 | 0.381 | 0.603 |
| `KRAS_RAF1_norfitness_6VJJ` | 245 | 0 | 245 | 0.127 | 0.829 | 0.968 | 0.381 | 0.603 |
| `KRAS_RALGDS-RBD_norfitness_1LFD` | 254 | 0 | 254 | 0.110 | 0.844 | 0.958 | 0.527 | 0.583 |
| `PSD95_CRIPT_1BE9` | 120 | 0 | 120 | 0.167 | 0.713 | 0.820 | 0.272 | 0.350 |
| `PSD95_Tm2F_1BE9` | 120 | 0 | 120 | 0.175 | 0.692 | 0.824 | 0.296 | 0.334 |
| `SARS2-RBD_ACE2_deltaKd_6M0J` | 791 | 0 | 791 | 0.054 | 0.885 | 0.948 | 0.248 | 0.273 |
| `hYAP65_peptide_FunctioncalScore_1JMQ` | 56 | 0 | 56 | 0.339 | 0.688 | 0.824 | 0.359 | 0.678 |
| **mean（14 assay 未加权）** | | | | 0.126 | **0.842** | **0.905** | **0.425** | **0.397** |
| *median* | | | | *0.125* | *0.859* | *0.918* | *0.420* | *0.377* |
...
3. 🔴 **细粒度明显更弱** —— 「多远」的 ρ_w 均值只有 **0.425**（within）/ **0.397**（LOAO），
...
| `ρ_w,within`（连续软标签） | `ρ_zeroshot` | +0.013 | 0.964 |
...
| assay | `ρ_zeroshot` | `AUC_within` | `ρ_w,within` |
|---|---:|---:|---:|
| `SARS2-RBD_ACE2_deltaKd_6M0J` | 0.6967 | 0.885 | 0.248 |
| `CD19_FMC63_Fitness_7URV` | 0.6032 | 0.911 | 0.554 |
| `KRAS_RALGDS-RBD_norfitness_1LFD` | 0.5872 | 0.844 | 0.527 |
| `KRAS_PICK3CG-RBD_norfitness_1HE8` | 0.5020 | 0.918 | 0.262 |
| `GB1_IgG-Fc_fitness_1FCC` | 0.5005 | 0.946 | 0.565 |
| `KRAS_RAF1_norfitness_6VJJ` | 0.4871 | 0.829 | 0.381 |
| `KRAS_RAF1-RBD_norfitness_6VJJ` | 0.4413 | 0.829 | 0.381 |
| `HLA-A2_TAPBPR_meanscore_5WER` | 0.4116 | 0.891 | 0.639 |
| `PSD95_CRIPT_1BE9` | 0.3672 | 0.713 | 0.272 |
| `ACE2_SARS2-RBD_enrich_6M17` | 0.2760 | 0.951 | 0.474 |
| `CXCR4_CXCL12_enrich_8U4O` | 0.2014 | 0.820 | 0.537 |
| `PSD95_Tm2F_1BE9` | 0.1944 | 0.692 | 0.296 |
| `5A12_Ang2_fitness_4ZFG` | 0.1074 | 0.875 | 0.458 |
| `hYAP65_peptide_FunctioncalScore_1JMQ` | 0.0886 | 0.688 | 0.359 |
| **mean** | **0.3903** | **0.842** | **0.425** |
...
**这条证据只覆盖「二值」那一层**：连续读出 `ρ_w,within` 与 `ρ_zeroshot` 的相关是
**+0.013, p=0.964** —— 基本就是 0。
⇒ **「encoder 把距离估得准不准」与「该 assay 的 zero-shot 好不好」之间，目前没有观察到任何关系。**

> 这不是回归目标的定义问题。有界软标签 `exp(−d/5Å)` 与原始距离 `d` 两种目标都测过，
> 相关分别是 **+0.013** 与 **−0.128**，**两者都在噪声范围内**。
> ⇒ 这个「没有关系」是数据本身的性质，不是指标选得不好。

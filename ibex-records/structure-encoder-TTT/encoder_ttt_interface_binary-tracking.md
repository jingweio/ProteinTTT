...
### 4.0 🔴🔴 **AdamW 的 weight_decay 未显式传 ⇒ 至今所有带训练的数字都受污染**（2026-09-15 发现）

**文档 §附录A 写的是 `weight_decay=0`（理由：正则由锚显式承担，不要两套正则互相干扰），
但代码里 `torch.optim.AdamW(params, lr=...)` 没传这个参数，而 PyTorch 的默认值是 `0.01`，不是 0。**

**受影响的运行**（全部 —— 只要带训练）：
`P1` / `P1b`（表征锚，已作废）、**`E-1` 四档 λ**、**`E-1n` 三个 null seed**、**`E-1a` anchor-only**。

**它为什么能解释全部结果**：`E-1a` 里 `probe_weight=0`，且分数锚在 `step 0` 实测 **3.48e-11**
⇒ 梯度≈0、**权重本不该动**；但 `|Δscore|max = 2.785`、Δρ = **+0.0115**。
能动它的只剩 weight decay（外加 Adam 会把极小梯度按 `m̂/√v̂` 归一化放大到 lr 量级）。
weight decay 与标签无关、与 probe loss 无关 ⇒ 真实标签 (+0.0118)、打乱标签 (+0.0104)、
无监督 (+0.0115) 三者几乎相同，**完全自洽**。

**修复**：加 `--weight_decay`，**默认 0**（与文档一致），两个 optimizer 都显式传。
**验证**：job 51897919 三臂分解 —— arm1 `probe0/wd0` 必须 ≈0；arm3 `probe1/wd0` 才是方法的真实效果；
arm4 `probe0/wd0.01` 用显式开关复现 `51897820`。

⚠️ **在 arm3 出来之前，本 project 的任何增益数字都不可引用。**
⚠️ **B project（`geometry-counterfactural-encoder-TTT`）不受影响** —— G-1/G-2 是零训练诊断，无 optimizer 参与。

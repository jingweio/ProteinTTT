...
- **2026-09-09** 补 §5.0：把**数据现状**（A1–A5）与**我们的实验结论**（G1）分开，并加口径对照表
  —— 此前「14 vs 23」在文中混用过。
- **2026-09-09** 更正措辞：硬编码 `c=−1` 不是「严格 zero-shot」，`c` 的符号来自生物物理先验 +
  我们在其它 assay DMS 上的确认（见 G1.5.1 的注）。
- **2026-09-09** G2 原动机（`p(aa|complex) − p(aa|monomer)` 的前置）**退役**，降优先级。

---

## 5. Results

### 5.0 先分清：哪些是**数据现状**，哪些是**我们的实验结论**

**A. 数据现状（关于 BindingGYM 与预训练模型本身，不是我们方法的结果）**
—— 全部来自 `../../local-records/binding-sites-overview/`：

| # | 现状 | 口径 |
|---|---|---|
| A1 | 真值 DMS 上，碰/不碰 binding-site 两组存在**方向一致的分布差异**（δ<0 在 16/16 个可检验 assay） | 16 assay（未排除那两个） |
| A2 | 但**效应量小**：OVL 中位 0.697、η² 中位 0.051；不碰界面那组仍保留全 assay IQR 的中位 95% | 同上 |
| A3 | **预训练 ProteinMPNN 建模不到这个区分**：δ<0 只在 8/16，另 8 个**符号翻转**；OVL 升到 0.844、η² 掉到 0.013 | 同上 |
| A4 | 逐 assay 的界面敏感度与真值**不相关**（ρ(δ_真值, δ_MPNN) = +0.376, p = 0.15） | 同上 |
| A5 | 榜单成绩与界面区分**无关**（Spearman(ρ, δ_MPNN) = +0.006, p = 0.98） | 同上 |

**B. 我们的实验结论（G1，本 project 产出）** —— 见 §5.1。

### 5.0.1 口径对照表（**不同实验用的 assay 集合不同，不要混引**）

| 实验 | 特征 | assay 集合 | 基线 | 为什么是这个集合 |
|---|---|---|---|---|
| G1 天花板 DP + 加性平移 + null | **二值** `1[d≤5Å]` | **14** | 0.3903 | 二值指示量在另 9 个 assay 里恒定（全碰或全不碰）⇒ 增益恒等于 0，无法定义 |
| G1b 逐 assay `c*` vs 共享 `c` | **二值** | **14** | 0.3903 | 同上 |
| G1c LOAO + 二值/连续**对比** | 二值 **与** 连续 | **14** | 0.3903 | 两个特征要在**同一集合**上比才公平 |
| **G1d 最终头条数字** | **连续** `exp(−d/5Å)` | **23** | **0.3939** | 连续距离在 23/23 上都有变化；23 也是已发表 0.397 / 0.42 的口径 |

⇒ **凡是引用「0.4505 / 0.4463 / +0.0566」，都是 23 assay、连续特征。**
凡是引用「天花板 0.4497」或「二值 +0.0432」，都是 14 assay、二值特征。
...
**「天花板」只对一个**明确写出的函数族**成立。** 本节界定的族是：
`s' = f(s, z)`，其中 `z` 是**二值**界面标签，且**同组成员必须拿到相同的校正** ——
因此**组内次序不可改**，唯一自由度是两组如何交错。
⚠️ **连续特征不属于这个族**（两个都碰界面但 `d` 不同的 variant 会拿到不同校正 ⇒ 组内次序也变），
所以它**不受这个天花板约束**，实测确实突破了（14 assay 上连续 oracle 0.4689 > 二值天花板 0.4497）。
更宽的族（任意 `g(s,d)`）没有有意义的上界 —— 自由度足够大时可以直接输出真值。

**DP 是什么。** 穷举「保持各自组内次序的合并方式」的数量是 C(n, n_A)，天文数字。
但最优合并有子结构：前 `i+j` 个位置的最优值只依赖 `(i, j)`。于是
`dp[i][j]` = 已从 A 放了 i 个、从 B 放了 j 个时 `Σ(位置 × 真值秩)` 的最大值，
转移就是「下一个位置从 A 取还是从 B 取」，复杂度 `O(n_A × n_B)`。
之所以最大化 `Σ(位置 × 真值秩)` 等价于最大化 Spearman：位置向量**无论怎么合并都是 1..n 的排列**，
均值方差固定，只有这个交叉项在变。

**为什么必须配随机标签 null。** DP 是**取最大**，所以即便标签毫无意义，它也能挑出一个
「碰巧比按 `s` 排序更吻合真值」的合并 —— 这是优化对样本的过拟合。
对照做法：把 `z` **随机置换**（组大小完全不变、组员身份无意义），跑同一个 DP。
它找到的增益就是「纯优化白拿的那部分」。**真正的信息 = 真标签的天花板 − 随机标签的天花板。**
实测：+0.0594 − 0.0123 = **+0.0471**。
...
> ⚠️ **`c` 的符号从哪来 —— 必须说清楚。** `c < 0` 编码的正是「碰界面的突变更伤结合」这个先验。
> 它有两个来源：**(i) 生物物理先验**（突变界面残基损害结合，无需任何数据）；
> **(ii) 我们在 DMS 上确认过**（现状 A1：16/16 个 assay 方向一致）。
> 所以「硬编码 `c=−1`」**对测试 assay 是 zero-shot**（没用它的任何 label），
> 但**不是「完全没有 DMS 知识介入」** —— 早先把它写成「严格 zero-shot」是**过强的说法，已更正**。
> LOAO 那一行是把这件事做规范的版本：`c` 只在其它 22 个 assay 上选，与官方 `inter_cluster` 同协议。
...
```
score' = score + c · sd(score) · exp( −d / 5Å )
```

| 符号 | 定义 |
|---|---|
| `score` | 预训练 ProteinMPNN 的 `global_score`（seed1 / M=5，在 WT 复合物上打分） |
| **`sd(score)`** | **该 assay 内「预训练 MPNN 预测分数」的标准差** —— **不是 DMS 的**。只用模型输出，不碰 label |
| `d` | 该 variant **所有突变位点**到另一个 entity 的**最小**重原子距离（多点突变取最近的那一个） |
| `c` | 一个标量。LOAO 选 ≈ −0.65 sd（全 23）；或直接硬编码 −1 |

**为什么要除以 `sd(score)`：** MPNN 的 `global_score` 是 mask 上 NLL 的求和、无长度归一化，
所以绝对尺度跨 assay 差极大（均值 −152 ~ −1829），sd 也差 **3.3×**（1.96 ~ 6.49）。
不换算成「该 assay 自己的 sd」，同一个 `c` 在不同 assay 上的实际强度完全不同、无法迁移。
...
### 5.2 G2 — monomer control（**已降优先级；原动机退役**）

> **2026-09-09 用户澄清：** `p(aa|complex) − p(aa|monomer)` 这个 decoder loss 只是**我们在做
> binding-sites 数据分析之前臆想的 draft plan**，不是既定方案。**不要被它带跑偏。**
> 现阶段应该只围绕**已经做出来的 binding-sites 数据分析**去细化 complexTTT。
>
> ⇒ G2 原本是**那个 loss 的可行性前置**，该动机随之退役。它剩下的价值是一个更弱的问题
> （「预训练模型的分数对 partner 有没有响应」），**不足以单独占用 1.5–2 GPU-h**。
> 若将来某个设计确实依赖「partner 通道里有信号」，再把它拉回来。
...
3. **换一个 rescoring 做不到的读数** —— 如 partner-conditional specificity。需先确认距离特征
   在该读数上能不能被拉开。
4. ~~先跑 G2 再决定~~ —— **已退役**（见 §5.2）。

**用户 2026-09-09 给的方向**：只围绕**已完成的 binding-sites 数据分析**（§5.0 的 A1–A5 + §5.1 的 G1）

...
| **ρ_5（5 档序数）** | **multinomial logistic regression** | **5 个档位**（按 `d` 的固定距离带，见下） | 预测的**期望档位**与真实档位之间的 **Spearman** |

**⇒ AUC 衡量「碰不碰」分得开不开；ρ_w 衡量「多远」估得准不准。**
...
**5 档序数读出（`ρ_5`）**：只问「碰不碰」太粗，只回归一个实数又把任务压成单一线性映射。
中间档位是把它拆成**有序的多分类**：

| 档位 | `d(r)` 范围 | `w = exp(−d/5Å)` 大致范围 | 含义 |
|---:|---|---|---|
| **0** | ≤ 5 Å | ≥ 0.37 | 界面本身 |
| 1 | 5 – 8 Å | 0.20 – 0.37 | 紧邻界面 |
| 2 | 8 – 12 Å | 0.09 – 0.20 | 第二层 |
| 3 | 12 – 20 Å | 0.02 – 0.09 | 外围 |
| 4 | > 20 Å | < 0.02 | 远端 |

- **用固定距离带、不用逐 assay 分位数** —— 这样「第 2 档」在每个复合物里含义相同，
  LOAO 的单一分类器才是良定义的。实测 **14/14 个 assay 的 5 档全部有样本**（逐 assay 占用见表）。
- 读出不取 `argmax`，而取**概率加权的期望档位** `Σ_k k · p_k` —— **保留序数结构**，
  否则相邻档位的错分与跨三档的错分会被同等对待。
- 另报 **±1 档准确率**（预测档位与真实档位相差不超过 1 的比例），用来看错得离不离谱。
...
**逐 assay 明细**（`L` = 复合物总残基数；`ρ_5` = 5 档序数读出）：

| assay | L | 排除 | 残基数 | 界面比例 | AUC within | AUC LOAO | ρ_w within | ρ_w LOAO | **ρ_5 within** | **ρ_5 LOAO** |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| `5A12_Ang2_fitness_4ZFG` | 652 | 4 | 648 | 0.056 | 0.875 | 0.879 | 0.458 | 0.100 | **0.459** | **0.163** |
| `ACE2_SARS2-RBD_enrich_6M17` | 931 | 0 | 931 | 0.046 | 0.951 | 0.970 | 0.474 | 0.322 | **0.543** | **0.415** |
| `CD19_FMC63_Fitness_7URV` | 497 | 52 | 445 | 0.092 | 0.911 | 0.886 | 0.554 | 0.204 | **0.583** | **0.188** |
| `CXCR4_CXCL12_enrich_8U4O` | 360 | 0 | 360 | 0.169 | 0.820 | 0.851 | 0.537 | 0.405 | **0.565** | **0.455** |
| `GB1_IgG-Fc_fitness_1FCC` | 262 | 0 | 262 | 0.137 | 0.946 | 0.940 | 0.565 | 0.468 | **0.473** | **0.509** |
| `HLA-A2_TAPBPR_meanscore_5WER` | 644 | 62 | 582 | 0.124 | 0.891 | 0.897 | 0.639 | 0.466 | **0.605** | **0.362** |
| `KRAS_PICK3CG-RBD_norfitness_1HE8` | 1107 | 192 | 915 | 0.038 | 0.918 | 0.946 | 0.262 | 0.166 | **0.447** | **0.281** |
| `KRAS_RAF1-RBD_norfitness_6VJJ` | 245 | 0 | 245 | 0.127 | 0.829 | 0.968 | 0.381 | 0.603 | **0.388** | **0.577** |
| `KRAS_RAF1_norfitness_6VJJ` | 245 | 0 | 245 | 0.127 | 0.829 | 0.968 | 0.381 | 0.603 | **0.388** | **0.577** |
| `KRAS_RALGDS-RBD_norfitness_1LFD` | 254 | 0 | 254 | 0.110 | 0.844 | 0.958 | 0.527 | 0.583 | **0.376** | **0.547** |
| `PSD95_CRIPT_1BE9` | 120 | 0 | 120 | 0.167 | 0.713 | 0.820 | 0.272 | 0.350 | **0.558** | **0.467** |
| `PSD95_Tm2F_1BE9` | 120 | 0 | 120 | 0.175 | 0.692 | 0.824 | 0.296 | 0.334 | **0.459** | **0.457** |
| `SARS2-RBD_ACE2_deltaKd_6M0J` | 791 | 0 | 791 | 0.054 | 0.885 | 0.948 | 0.248 | 0.273 | **0.295** | **0.326** |
| `hYAP65_peptide_FunctioncalScore_1JMQ` | 56 | 0 | 56 | 0.339 | 0.688 | 0.824 | 0.359 | 0.678 | **0.566** | **0.718** |
| **mean** | | | | 0.126 | **0.842** | **0.905** | **0.425** | **0.397** | **0.479** | **0.432** |
| *median* | | | | *0.125* | *0.859* | *0.918* | *0.420* | *0.377* | *0.466* | *0.456* |
...
3. 🔴 **细粒度明显更弱** —— 「碰不碰」的 AUC 是 **0.842 / 0.905**，
   而「多远」无论怎么问都低得多：5 档序数 **0.479 / 0.432**、
   连续软标签 **0.425 / 0.397**。
   **这正是 structure-TTT 可能有空间的地方。**
4. **分档读出优于连续回归**（within 0.479 vs 0.425；
   LOAO 0.432 vs 0.397）。
   合理 —— 序数分类器可以给每一档学各自的边界，ridge 只有一个线性映射。
   ⇒ **报「encoder 的细粒度能力」时应当用分档读出，用连续回归会低估它。**
   ±1 档准确率 **0.833**（within）/ **0.793**（LOAO），
   说明大部分错分只差一档、不是乱猜。
...
| `ρ_5,within`（5 档） | `ρ_zeroshot` | **-0.418** | 0.137 | **-0.478** | 0.084 |
...
⚠️ 这里同时看了 3 个探针指标，**严格来说还要做多重比较校正**（BH），校正后 0.057 会更不显著。

**逐 assay 明细**（按 zero-shot 从高到低）：

| assay | `ρ_zeroshot` | `AUC_within` | `ρ_5,within` | `ρ_w,within` |
|---|---:|---:|---:|---:|
| `SARS2-RBD_ACE2_deltaKd_6M0J` | 0.6967 | 0.885 | 0.295 | 0.248 |
| `CD19_FMC63_Fitness_7URV` | 0.6032 | 0.911 | 0.583 | 0.554 |
| `KRAS_RALGDS-RBD_norfitness_1LFD` | 0.5872 | 0.844 | 0.376 | 0.527 |
| `KRAS_PICK3CG-RBD_norfitness_1HE8` | 0.5020 | 0.918 | 0.447 | 0.262 |
| `GB1_IgG-Fc_fitness_1FCC` | 0.5005 | 0.946 | 0.473 | 0.565 |
| `KRAS_RAF1_norfitness_6VJJ` | 0.4871 | 0.829 | 0.388 | 0.381 |
| `KRAS_RAF1-RBD_norfitness_6VJJ` | 0.4413 | 0.829 | 0.388 | 0.381 |
| `HLA-A2_TAPBPR_meanscore_5WER` | 0.4116 | 0.891 | 0.605 | 0.639 |
| `PSD95_CRIPT_1BE9` | 0.3672 | 0.713 | 0.558 | 0.272 |
| `ACE2_SARS2-RBD_enrich_6M17` | 0.2760 | 0.951 | 0.543 | 0.474 |
| `CXCR4_CXCL12_enrich_8U4O` | 0.2014 | 0.820 | 0.565 | 0.537 |
| `PSD95_Tm2F_1BE9` | 0.1944 | 0.692 | 0.459 | 0.296 |
| `5A12_Ang2_fitness_4ZFG` | 0.1074 | 0.875 | 0.459 | 0.458 |
| `hYAP65_peptide_FunctioncalScore_1JMQ` | 0.0886 | 0.688 | 0.566 | 0.359 |
| **mean** | **0.3903** | **0.842** | **0.479** | **0.425** |
...
**这条证据只覆盖「二值」那一层。** 细粒度读出与 `ρ_zeroshot` 之间不但没有正相关，
**符号还是反的**：5 档序数 **−0.418 (Spearman) / −0.478 (Pearson)**，
连续软标签 **+0.013 / +0.004**。

> **三种细粒度写法都测过了**：原始距离 `d`（−0.128）、有界软标签 `exp(−d/5Å)`（+0.013）、
> 5 档序数（−0.418）。**没有一个给出显著的正相关。**
> ⇒ 这不是「细粒度任务定义没选好」造成的，而是数据本身如此。
>
> 5 档那个 −0.418 (p=0.137) 值得留意但**不宜过度解读**：n=14、未过显著、也未做多重比较校正。
> 若它是真的，含义会是「encoder 把距离层次分得越清楚的 assay，ProteinMPNN 反而打分越差」——
> 这更可能反映某个混淆项（例如**残基数多、界面占比低的大复合物**同时让分档更容易、
> 让 DMS 排序更难），而不是一个因果机制。

...
| 2026-09-15 | **E-1n permutation null** | **51897490** | ~9 min | λ=100，界面标签置换，3 seed，3 pilot assay | `data/e1n_perm*_per_assay.csv` | 🔴 **null 拿到真实增益的 88%** |
...
**E-1n null 结果（λ=100，界面标签在有效残基间置换，π 与 w_pos 保持不变）**

| permute seed | mean Δρ | wins |
|---:|---:|:--:|
| 0 | +0.0130 | 3/3 |
| 1 | +0.0064 | 3/3 |
| 2 | +0.0117 | 3/3 |
...
🔴 **null / 真实 = 0.0104 / 0.0118 = 88%。** 把界面标签完全打乱，增益几乎不变。
⇒ **E-1 的增益基本与界面先验无关**；界面的净贡献 ≈ **+0.0014**，落在噪声内（配对 sd 0.0037）。

- 与「`AP_norm` 与 Δρ 反向」的观察**一致** —— 当时已看出增益不来自「界面更可分」，null 予以证实。
- **与 decoder 侧完全相反**：那条线的 null 只拿到 19%（真实 +0.1594 vs null +0.0298，81% 归于界面先验）。
- ⇒ 增益的真实来源**仍然未知**。候选：probe 梯度（无论标签是什么）＋分数锚共同造成的某种隐式平滑/正则。
  要定位它需要更极端的对照，例如**只留 anchor、去掉 probe loss**（预期 Δρ=0），
  或把 probe 目标换成与结构完全无关的量。
...
### 4.0 🔴🔴 **观察到的增益不来自方法的任何组件**（2026-09-15，E-1d/E-1e）

**四臂分解（3 pilot assay，lr=1e-4，steps=150，λ=100）：**

| arm | probe loss | weight decay | mean Δρ | wins |
|---|:--:|:--:|---:|:--:|
| E-1 原始 | ✅ | 0.01（意外开启） | +0.0118 | 3/3 |
| **arm1 什么都没有** | ❌ | **0** | **+0.0108** | 2/3 |
| arm3 方法本身 | ✅ | 0 | +0.0118 | 3/3 |
| arm4 只有 decay | ❌ | 0.01 | +0.0115 | 3/3 |
| *E-1n 打乱标签* | *置换* | *0.01* | *+0.0104* | *3/3* |

🔴 **全部落在 +0.0104 ~ +0.0118。有监督、乱监督、无监督、有无衰减，结果一样。**
⇒ **增益不来自 probe loss，也不来自 weight decay，而来自「跑了 150 步优化」这件事本身。**

**逐 assay 的 `|Δscore|max` 揭示了机制线索**：

| arm | `5A12_Ang2` | `PSD95_CRIPT` | `CD19_FMC63` |
|---|---:|---:|---:|
| arm1（probe0 / wd0） | **6.1e-05** | **2.790** | **1.648** |
| arm4（probe0 / wd0.01） | 0.698 | 2.785 | 1.645 |

PSD95 与 CD19 在两臂几乎**逐位相同** ⇒ **weight decay 对它们毫无作用**；
只有 `5A12_Ang2` 被 decay 影响（6.1e-05 → 0.698）。

**剩下的嫌疑：Adam 在近零梯度上的数值行为。** 分数锚在 `step 0` 实测 **3.48e-11**，
梯度近零；但 Adam 的更新是 `lr·m̂/(√v̂+eps)`，**它把梯度幅度归一化掉了** ——
只要数值噪声有一致方向，就能产出 lr 量级的步长，150 步累积成实质改动。
这也解释了 assay 间差异（是否存在一致方向因 assay 而异）。
**判定实验**：job 51898314，`lr=0`（必须 Δρ=0）与 `lr=1e-6`（若是 Adam 归一化，增益应当仍在）。

#### ⚠️ 自我更正：此前把原因归给 weight_decay，是错的
2026-09-15 早些时候我断言「`AdamW` 的 `weight_decay` 默认 0.01 而文档写 0，这解释了全部结果」。
**arm1 推翻了它** —— 关掉 decay 后 PSD95/CD19 照样动、且与开着时数值几乎相同。
`weight_decay` 与文档不符**确实是一个真实的实现 bug**（已修，加 `--weight_decay` 默认 0，两个 optimizer 都显式传），
但**它不是增益的成因**。归因不能只看「哪个可疑组件存在」，要看关掉它之后结果变不变。

...
| **G-2** | 51897764 | ✅ **slide/pull = 1.12~1.87**，slide 保持接触（100–126%）而 pull 打到 12% ⇒ **模型建模的是配对，不只是接触** |
...
## 6. ✅ 已解除的混淆（G-2，2026-09-15）

原担心：**大幅扰动的 gap 主要来自「不再接触」，而非「pair-specific 配对被改变」。**
**G-2 否定了这个担心** —— `slide`（保持间距与接触、只改配对）的 gap 在所有幅度上
**都不小于** `pull`（拉开间距），slide/pull = 1.12~1.87。⇒ 模型确实在建模配对关系。
⚠️ 但逐 assay 异质：1 Å 下 9/14 是 slide 更敏感，`HLA-A2_TAPBPR` 对 slide 完全无反应（−0.05 sd）。

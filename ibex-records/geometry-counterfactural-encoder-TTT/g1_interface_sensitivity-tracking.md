...
| 2026-09-15 | **G-2 pull vs slide** | **51897764** | **0:41** | `--reps 5 --mags 0.5 1 2 4`，14 assay | `data/g2_pull_vs_slide.csv` | ✅ **slide ≥ pull ⇒ 前提成立** |

**G-2 结果（14 assay）** —— `pull` 沿质心轴（改变间距），`slide` 垂直于轴（保持间距、只换配对）

| 幅度 | pull (sd) | slide (sd) | **slide/pull** | pull 接触% | slide 接触% |
|---:|---:|---:|---:|---:|---:|
| 0.5 Å | +0.176 | +0.329 | **1.87** | 86.2% | 100.6% |
| 1.0 Å | +1.137 | +1.269 | **1.12** | 74.1% | 102.9% |
| 2.0 Å | +2.856 | +3.696 | **1.29** | 50.6% | 107.9% |
| 4.0 Å | +3.805 | +6.008 | **1.58** | 12.2% | 125.6% |

✅ **slide 在所有幅度上都 ≥ pull，且 slide 几乎不损失接触**（100–126%，CA–CA 8 Å 代理），
而 pull 把接触打到 12%。⇒ **模型对「谁和谁配对」比对「还接不接触」更敏感**
⇒ organized §6 的混淆解除，**counterfactual 的前提成立**。

🔴 **但逐 assay 有异质性**（1 Å）：**9/14** 是 slide 更敏感，另 5 个反过来。
`HLA-A2_TAPBPR` 的 slide 甚至是 **−0.05 sd**（对滑动完全无反应）。
⇒ 均值成立，**不是每个复合物都成立**；建模时这 5 个 assay 可能需要不同的扰动族。

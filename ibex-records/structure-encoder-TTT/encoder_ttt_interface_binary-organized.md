...
**状态：T0 通过；P1（λ 扫描）跑完，四档全无增益；P1b 归因诊断在跑。**

| 阶段 | job | 结论 |
|---|---|---|
| **T0** no-op gate | 51886003 | ✅ 四项全过：cu117 可用、ckpt md5、`steps=0` 分数逐行不变、baseline ρ 与换平台前记录差 ≤0.0020 |
| **P1** λ ∈ {1,10,100,1000} | 51886700 | ⚠️ **mean Δρ 全在 ±0.0014 内**，改动最大的 λ=1 最差（−0.0014）。但**都在噪声内**，与 0 不可区分 |
| **P1b** 前后独立探针 | 51887033 | 🔴 **`AP_norm` 0.464 → ~1.000（理论最大值），而 ρ 不动** |

## 🔴 本轮的结论：这条注入路径失效，且失效机制已经定位

**训练把界面可分性推到了理论最大值，zero-shot 完全不动。**

| assay | `AP_norm` before | after | Δρ (λ=10) |
|---|---:|---:|---:|
| `5A12_Ang2_fitness_4ZFG` | 0.4652 | **1.0000** | +0.0005 |
| `PSD95_CRIPT_1BE9` | 0.2929 | **1.0000** | +0.0012 |
| `CD19_FMC63_Fitness_7URV` | 0.6339 | **1.0000** | −0.0016 |
| **mean** | **0.4640** | **1.0000** | **+0.0000** |

**机制：encoder 把界面标签编码进了一个冻结 decoder 几乎读不到的子空间。**
证据是 λ=100 那一档 —— `anchor_V ≈ 1.0e-4`（每元素平均平方偏移）⇒ `h_V` 每元素 **RMS 偏移仅 ≈0.01**，
而 `h_V` 过 LayerNorm 后量级是 O(1)。**约 1% 的表征扰动就让 `AP_norm` 达到 0.998，
同时 `|Δscore|max` 只有 0.34、Δρ = −0.0002。**

为什么必然如此：`h_V` 是 128 维而标签是二值，**线性可分只需要一个方向**；decoder 的权重是**预训练冻结**的，
只对其中部分方向敏感。优化目标 = probe loss + λ·anchor，**最省力的解就是在 decoder 不敏感的方向上编码标签** ——
probe loss 降到 0、anchor 也小、分数不变，三者同时满足。
🔴 **anchor 惩罚的是改动的「大小」，不是改动的「方向」，它无法区分「影响 decoder 的改动」与「不影响的改动」。**

### 因此能说 / 不能说

**能说**：
1. 在 encoder 上加界面二分类的辅助监督，可把 `AP_norm,within` 推到 ~1.000，**而 zero-shot ρ 不变**。
2. 这只需要 ~1% 的表征扰动。
3. ⇒ **proposal §3 那条 `AP_norm,within` vs `ρ_zeroshot` 的相关（Spearman +0.515, p=0.060）不支持因果方向**：
   干预性地把前者推满，后者不动。这比原来那条观察性相关更有力，方向相反。
4. ⇒ **任何只作用于 encoder 表征、而不检查 decoder 输出的辅助目标，都可能被这条「零空间作弊」路径绕过。**

**不能说**：
1. ❌「encoder 的界面表征与 zero-shot 无关」—— 只否证了**这一种**提升方式。
   §3 的相关仍可能由第三因素驱动（结构质量 / 复合物大小 `L` / assay 噪声，proposal §3.3 自列的三个混淆项），
   我们并没有重现那个因素。
2. ❌ n=3，且是非随机的 pilot assay；只测了 `lr=1e-4, steps=150` 一组。
3. ⚠️ **`AP_norm=1.0` 未必是「学到可泛化的界面几何」**，也可能是「把这 L 个残基的标签记住了」——
   探针虽是 5-fold CV，但 **encoder 在全部残基上训练过，标签信息已经进了 `h_V`**，
   所以这个 CV **检测不到 encoder 层面的过拟合**。这不影响上面的结论（无论记住还是学到，结果都是 ρ 不动），
   但它意味着 **P1b 并不像设计时以为的那样「独立」**。

### 这改变了后续计划

按「先估上界，上界低则后面工作可省」：**沿「提升 `h_V` 界面线性可分性」这条路的上界已经测到 —— 把那个量推到
理论最大值 1.0，收益是 0。** 因此：

- ❌ **P2（扫 lr × steps）失去意义** —— 不论怎么调，都在同一条已证无效的路径上。
- ❌ **M1 / M2（14-assay 主实验 + null）失去意义** —— 它们测的就是这条路径。
- ✅ **要改的是注入方式，不是超参。** 关键约束：**目标函数里必须有一项直接关联 decoder 的输出**，
  否则零空间作弊会重演。§7（edge 视角）此前被搁置，但本结论改变了它的前提 —— 尤其是
  「`h_E` 不在 probe 视野内却直达 decoder」那一条，现在有了实证意义。
...
| **P1** | **先定 λ 的量级** | lr=1e-4、steps=150 固定，**λ ∈ {1, 10, 100, 1000}** | 3 个 pilot assay |
| **P2** | 再定 lr × steps | P1 选出的 λ，lr ∈ {3e-5, 1e-4, 3e-4} × steps ∈ {50, 150, 400} | 3 个 pilot assay |

> **P1/P2 的顺序是 2026-09-14 调换过的**（原为先 lr×steps、λ=1 固定）。
> 本地 smoke 显示 **λ=1 几乎不构成约束**：probe loss 60 步从 0.1303 掉到 0.0020（两个数量级），
> 而 anchor 两项只到 2.6e-3 / 3.6e-4。在一个近乎无约束的设定下扫 lr 与 steps，选出来的"最优"
> 多半只是"训得最少的那个"——**P1 的结论会被 λ 的错误取值污染，P2 再调 λ 就建立在一个坏的 (lr, steps) 上。**
> λ 的候选范围也随之整体上移（原 `{0.1, 1, 10}`）。

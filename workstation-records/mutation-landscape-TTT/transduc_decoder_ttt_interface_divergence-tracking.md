...
**2026-09-13 12:46 起：workstation 上没有本任务的进程在跑，GPU util 0%。**
...
5. **`sweep_M=1` 的 ρ 与 `sweep_M=5` 的不可直接比**（同一 λ=0.03，ACE2 分别是 0.3649 / 0.3237）。
   只有最后的 M=5 复评可比。**跨运行拼 λ 曲线 = 混口径**，organized §4.3 的表因此
   限定「每个 assay 只用一次运行」。
6. **训练 M 从未作为超参扫过**，但它有效果：`GB1` 在 M=5 训练下 0.5645，M=1 训练下 0.5577。

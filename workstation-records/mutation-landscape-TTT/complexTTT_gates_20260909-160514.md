...
- ⚠️ **更要紧的是：本文的增益是在同一个分数向量上做的配对比较**，seed 噪声在配对中大部分会抵消。
  ...
  **可测且大概率 0 GPU**：workstation 上 `seed2_M5`–`seed5_M5` 各有 **10 个 assay 的 per-variant 分数**
  ...
  ⚠️ **但「这 10 个是哪 10 个、与本文 14 个工作集重叠几个」尚未核实** —— 2026-09-10 尝试时
  workstation `10.67.24.41` 连不上（banner exchange 超时）。若重叠很小，这个检查就没那么便宜。
  在此之前，**增益的误差棒是未知的**，只能说它远大于 baseline 自身的 σ。

...
**为什么二值读出不能只看 ROC-AUC**：界面残基是**少数类** ——
14 个 assay 的界面占比只有 **12.6%**（最低 `KRAS_PICK3CG-RBD` 仅 3.8%）。
ROC-AUC 在负类占绝对多数时**偏乐观**：它只问「正样本是否普遍排在负样本前面」，
一个产生大量假阳性的探针照样能拿到高 AUC，因为每个假阳性只稀释一点点。

补一个对不平衡敏感的指标 —— **average precision（AP，precision-recall 曲线下面积）**。
AP 受正类占比直接影响（随机探针的 AP **就等于**界面占比），所以**不同 assay 之间不可直接比**。
因此报**归一化版本**：

```
AP_norm = ( AP − 界面占比 ) / ( 1 − 界面占比 )        0 = 随机探针，1 = 完美
```

这样各 assay 可比，且 0 点有明确含义。

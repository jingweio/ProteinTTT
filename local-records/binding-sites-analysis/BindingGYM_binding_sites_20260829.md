...
| 界面定义**站得住吗**？ | 形式定义在 **§2.0**，8 条前提在 **§2.1**（带 assert 的脚本）。划分是 **entity-based、与 mutation 无关**，由**元数据**定。**原定义有 bug**：它把「到结构里**任意其它链**的距离 ≤ 5 Å」当作界面，于是抗体的 **VH–VL packing 也被算成结合界面**；改掉后 assay 均值 **0.4295 → 0.4697**，variant 加权 **0.4764**。 |
...
设一个 assay 对应唯一一份 WT 复合物结构 `S`（`input/structures/` 里的 `pdb_file`），其链集合为 `C(S)`。

**① entity 划分.** 把 `C(S)` 划分成两个非空、不交、并集为 `C(S)` 的 **binding entity**：

```
E1 ∪ E2 = C(S) ,   E1 ∩ E2 = ∅ ,   E1 ≠ ∅ ,   E2 ≠ ∅
```

划分**只由元数据决定**（`DMS_id` 命名的两个 binding partner），**与 mutation 数据无关**。
`|C(S)| = 2` 时划分唯一确定（22/25 个 assay）；`|C(S)| = 3` 时由 curated 表给出（3/25，见 §2.2(b)）。

**② 原子集合.** 对残基 `r`，记 `A(r)` = `r` 的**全部非氢原子**（backbone 与 side chain 都算，
按 PDB 第 77–78 列的元素判定，不靠原子名猜）。对链集合 `X`，记 `A(X)` = `X` 中所有残基的重原子并集。

**③ 残基级判据.** 残基 `r` 属于 entity `E_i`。定义它到**另一个** entity 的距离为最小重原子对距离：

```
d(r) = min { ‖ x_a − x_b ‖₂ : a ∈ A(r), b ∈ A(E_other) }      # E_other = 另一个 entity
```

则

```
r ∈ binding-sites(S)   ⟺   d(r) ≤ θ
```

主口径 **θ = 5.0 Å**。

> `x_a` 取结构文件里**沉积的坐标本身**：不生成对称拷贝、不考虑晶体堆积、不做任何能量最小化。
> 无坐标的残基 `d(r)` 未定义，**按定义不属于** binding-sites 集合（见 §2.1 的覆盖率）。

**④ 被排除的接触（关键）.** 由 ③ 中 `b ∈ A(E_other)` 这一限定直接可得，以下**都不计入**：
- **同一 entity 内的链间接触** —— 如 Fab 的 **VH–VL packing**（这正是原口径 D0 的 bug）
- **链内接触** —— 同一条链上的其它残基
- 任何非蛋白原子（本 benchmark 中不存在，见 §2.1）

**⑤ variant 级标签.** variant `v` 的突变位点集合 `M(v)`（跨所有链，逐链 1-based 序列位置）。

```
iface(v) = 1   ⟺   ∃ p ∈ M(v)  使得  π(p) ∈ binding-sites(S)
```

其中 `π` 是「序列位置 → 结构残基」的映射（§2.4，双路交叉验证 0 冲突）。
即**只要有任一突变位点落在界面上就算「碰界面」**，与突变个数无关。
`M(v) = ∅` 的 WT 行**未定义**，从所有分析中剔除（22 行，见 §6）。

### 2.1 前提（22 个结构逐条核实，脚本 `s14_preconditions.py` 带 assert）

| 前提 | 实测 | 为什么定义依赖它 |
|---|---|---|
| 记录类型 | 仅 `ATOM` / `TER` / `END`（142,057 / 47 / 22 行），**HETATM = 0** | **没有配体、水、金属** ⇒ ②里不需要"非蛋白原子"策略。**注意：KRAS 复合物里因此没有 GDP/GTP 也没有 Mg²⁺**，核苷酸介导的接触无法体现 |
| altLoc（第 17 列） | 22/22 全为空 | 不需要选构象；每个残基的 `A(r)` 唯一 |
| 元素列（77–78） | 空值 **0** 行 | ②的"非氢"判定是**按元素**精确执行，不靠原子名前缀猜 |
| 残基名 | 只有 **20 种标准氨基酸**（无 MSE / 无修饰） | 三字母→一字母映射无 `X` 回退 ⇒ §2.4 的序列比对在干净字母表上做 |
| insertion code（第 27 列） | 仅 `4ZFF` / `4ZFG` 用（Kabat，`A/B/C`） | 残基 id 必须是**字符串** `resSeq+iCode`（如 `52A`）；纯 `int(line[22:26])` 会把 `52` 与 `52A` 并成一个残基 |
| 链的存在性 | `wildtype_sequence` 的 53 个 (assay, chain) **全部**在对应结构里 | ①的划分覆盖结构里的每一条链 |
| 坐标覆盖率 | 9,931 个序列位点中 **9,493 有坐标**，438 无（**4.41%**），12/53 条链有缺口 | ③里"无坐标 ⇒ 不属于界面"这条策略的适用范围 |
| **文库位点的覆盖率** | **无坐标的文库位点 = 0** | ⇒ **每一个被打分的突变都是可判定的**，⑤不会因缺坐标而产生假的"不碰界面" |

**θ 的选取不是从数据里学的**，是结构生物学里 van der Waals 接触的常规截断；
它的敏感性在 §2.3 逐个报告，结论在 4–6 Å 区间内稳定。
...
> 它的含义就是「到另一个 entity 的最小重原子距离」（数值未变，见 §2.2(b) 的回归 gate）。

### 2.3 对照定义与 cutoff 敏感性

对照的另外两个定义：

| id | 定义 | assay 均值 | variant 加权 |
|---|---|---|---|
| **D0** | 到**任意其它链** ≤ 5 Å（**原 plan 的口径**，把同 entity 内的链间接触也算进去） | **0.4295** | 0.3968（149,368 / 376,424） |
| **D1** | 到**另一个 entity** ≤ 5 Å（**本文主口径**，§2.0） | **0.4697** | 0.4764（179,330 / 376,424） |
...
结果：**25/25 assay、0 处冲突** —— Table A 里每一个位点号、以及每一个 variant 的界面标签，都靠这一条。
（坐标覆盖率与 insertion code 见 §2.1，不在此重复。）
...
### 2.6 多链复合物怎么算：以 `4D5_HER2_1N8Z` 为例
...
scripts/binding_sites/
  ...
  s14_preconditions.py      §2.1 的 8 条前提，全部带 assert

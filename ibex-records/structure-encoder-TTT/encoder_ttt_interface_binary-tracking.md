...
| 2026-09-14 | **P1 λ 扫描** | **51886700** | **8:33** | lr=1e-4 steps=150 固定，λ ∈ {1,10,100,1000}，3 个 pilot assay | `data/p1_lam*_per_assay.csv` | ⚠️ **四档全无增益** |
| 2026-09-14 | **P1b 前后独立探针** | **51887033** | **6:24** | 同上 λ ∈ {1,10,100} + `--probe_qa` | `data/p1b_lam*_per_assay.csv` | 🔴 **AP_norm→1.0 而 ρ 不动** |

**P1b 结果与末步 anchor（每元素平均平方偏移）**

| λ | mean AP_norm before→after | mean Δρ | `anchor_V` 末步 | `h_V` RMS 偏移 | \|Δscore\|max |
|---:|---|---:|---:|---:|---:|
| 1 | 0.4640 → **1.0000** | −0.0014 | 1.2~1.8e-3 | ≈0.04 | 7.38 |
| 10 | 0.4640 → **1.0000** | +0.0000 | 3.5~4.9e-4 | ≈0.02 | 1.54 |
| 100 | 0.4640 → **0.9982** | −0.0002 | 0.9~1.1e-4 | **≈0.01** | 0.34 |

🔴 **λ=100 是最有说服力的一档**：`h_V` 每元素 RMS 只动约 **0.01**（LayerNorm 后量级 O(1) ⇒ ~1%），
`AP_norm` 已达 0.998，而 Δρ = −0.0002、`|Δscore|max` = 0.34。
⇒ **encoder 把界面标签编码进了冻结 decoder 几乎读不到的方向。**
anchor 只惩罚改动的**幅度**、不管**方向**，因而拦不住这种「零空间作弊」。

⚠️ **P1b 的设计缺陷（自我更正）**：它本意是「独立」验证，但 **encoder 是在全部残基上训练的，
标签信息已经进入 `h_V`**，探针的 5-fold 只是 `h_V` 内部的留出，不是标签的留出
⇒ **它检测不到 encoder 层面的过拟合**，`AP_norm=1.0` 无法区分「学到几何」与「记住标签」。
这不动摇「ρ 不动」这个结论，但 P1b 不能被称作对表征泛化性的独立检验。

**P1 逐档结果（Δρ = rho_ttt − rho_base）**

| λ | `5A12_Ang2` | `PSD95_CRIPT` | `CD19_FMC63` | mean Δρ | wins | \|Δscore\|max |
|---:|---:|---:|---:|---:|:--:|---:|
| 1 | +0.0040 | −0.0026 | −0.0056 | **−0.0014** | 1/3 | 7.38 |
| 10 | +0.0005 | +0.0012 | −0.0016 | **+0.0000** | 2/3 | 1.54 |
| 100 | +0.0001 | +0.0010 | −0.0018 | **−0.0002** | 0.34 | 0.34 |
| 1000 | −0.0003 | +0.0004 | −0.0008 | **−0.0002** | 1/3 | 0.27 |

- **anchor 确实在工作**：`|Δscore|max` 随 λ 单调缩小（7.38 → 1.54 → 0.34 → 0.27），λ→∞ 即「不改」。
- **改动越大越差**：唯一显著改动的那档（λ=1）给出最差的 mean Δρ。
- 🔴 **但三档的 Δρ 都在 ±0.006 以内，而配对比较的噪声 sd 是 0.0037**
  ⇒ **统计上与 0 不可区分**；n=3 也检验不出任何东西。**不能写成「λ=1 有害」。**

**能说的**：在 `lr=1e-4, steps=150`、3 个 pilot assay 上，λ 从 1 到 1000 **没有任何一档产生正的平均增益**。
**不能说**：H 被否证 —— 见下条。

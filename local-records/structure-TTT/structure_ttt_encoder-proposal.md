...
具体地：取 §2 线性探针学到的权重方向 `u`（128 维，`h_V` 空间里的「界面方向」），令

```
h_V'(r) = h_V(r) + α · ‖h_V‖ · u        α 扫过正负两侧
```

**🔴 但单独跑这一条读不出任何东西，必须配对照** —— 因为**只要扰动幅度够大，
沿任何方向改 `h_V` 都会让模型变差**。没有对照就分不清
「沿界面方向改」和「改 `h_V` 这件事本身」。至少要有这两条对照臂：

| 臂 | 是什么 | 它排除什么 |
|---|---|---|
| **界面方向 `+u` / `−u`** | 主臂。两个符号都要扫 —— 一侧应当「加强界面感知」，另一侧「削弱/反转」 | — |
| **随机方向**（同幅度，多个随机种子） | 在 `h_V` 空间里取随机单位向量替代 `u` | **排除「任何扰动都会有这个效果」** |
| **打乱的界面方向** | 把 `u` 的 128 个分量随机置换后再用 | 保留 `u` 的数值分布、只破坏它与各维度的对应关系 |

**幅度必须匹配** —— 三条臂要在同一个可比的尺度上比较（例如令 `‖Δh_V‖` 相等，
或令它们引起的分数标准差变化相等），否则比的是幅度不是方向。
128 维空间里随机方向与 `u` 近似正交，所以随机臂是一个干净的 null。

**预期的signature**：主臂应当**关于 α 不对称**（一侧改善、另一侧恶化），
而随机臂应当**关于 α 近似对称且只会恶化**。
